...
/** Brand perception map — from brand_perception_map_more_green.html */
export function BrandPerceptionVisual({ className = '' }: { className?: string }) {
  const pathRef = useRef<SVGPathElement>(null);
  const compRef = useRef<SVGCircleElement>(null);
  const openRef = useRef<SVGCircleElement>(null);
  const idealRef = useRef<SVGCircleElement>(null);
  const idealTxtRef = useRef<SVGTextElement>(null);
  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      const idealTxt = idealTxtRef.current;
      if (!path || !comp || !open || !ideal || !idealTxt) return;

      path.style.transition = 'none';
      path.setAttribute('stroke-dashoffset', '200');
      for (const el of [comp, open, ideal, idealTxt]) {
      ...
      timers.push(
        window.setTimeout(() => {
          comp.style.transition = 'opacity .5s';
          comp.setAttribute('opacity', '1');
        }, 200),
      );
      timers.push(
        window.setTimeout(() => {
          open.style.transition = 'opacity .5s';
          open.setAttribute('opacity', '1');
        }, 500),
      );
      timers.push(
        window.setTimeout(() => {
          ideal.style.transition = 'opacity .5s';
          ideal.setAttribute('opacity', '1');
          idealTxt.style.transition = 'opacity .5s';
          idealTxt.setAttribute('opacity', '1');
        }, 800),
      );
      timers.push(
        window.setTimeout(() => {
  ...
  return (
    <div
      className={`flex aspect-square size-full items-center justify-center bg-[var(--v2-blue)] p-5 sm:p-6 ${className}`}
      aria-hidden
    >
      <div className="w-full max-w-[340px] rounded-[20px] border-2 border-[var(--v2-ink)] bg-[var(--v2-lime)] p-5 shadow-[8px_8px_0_var(--v2-ink)] sm:p-6">
        <div className="mb-2.5 flex items-center justify-between border-b border-[var(--v2-ink)]/15 pb-2.5">
          <QareWordmark />
          <span className="font-mono text-[9px] tracking-[0.03em] text-[#333]">
            INTERACTIVE REPORT
          </span>
        </div>

        <p className="mb-0.5 text-[12.5px] font-extrabold tracking-[0.04em] text-[var(--v2-ink)]">
          BRAND PERCEPTION MAP
        </p>
        <p className="mb-3.5 text-sm font-semibold text-[#333]">Relevance × Differentiation</p>

        <div className="overflow-hidden rounded-[14px] border-2 border-[var(--v2-ink)] bg-white p-2.5">
          <svg viewBox="0 0 280 190" className="h-auto w-full" role="img">
            <rect x="140" y="10" width="130" height="85" fill="#C9FF6E" />
            <line
              x1="140"
              y1="10"
              x2="140"
            ...
            <line
              ...
              strokeOpacity="0.15"
            />
            <text x="18" y="26" fontSize="10.5" fontWeight="600" fill="#8a94a3">
              High relevance
            </text>
            <text x="150" y="172" fontSize="10.5" fontWeight="600" fill="#111">
              High differentiation
            </text>
            <path
              ref={pathRef}
              d="M78 128 C110 100, 150 65, 195 55"
              fill="none"
              stroke="#111"
              strokeWidth="4"
              strokeLinecap="round"
            ...
            />
            <circle cx="78" cy="128" r="16" fill="#C9FF6E" stroke="#111" strokeWidth="2" />
            <text
              x="78"
              y="132"
              fontSize="10"
