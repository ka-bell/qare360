...
/** Hard-outline concept test — concept_test_qare_hard_outline_final.html */
export function ConceptTestVisual({ className = '' }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const reset = () => {
      if (nRef.current) nRef.current.textContent = '0';
      if (aRef.current) aRef.current.textContent = '0%';
      if (bRef.current) bRef.current.textContent = '0%';
      if (aBarRef.current) aBarRef.current.style.width = '0%';
      if (bBarRef.current) bBarRef.current.style.width = '0%';
      setShowRec(false);
    };
    ...
    const run = () => {
      ...
      timers.length = 0;
      reset();
      timers.push(window.setTimeout(() => animateCount(nRef.current, 412, 900), 150));
      timers.push(
        window.setTimeout(() => {
          animateCount(aRef.current, 38, 1000, '%');
          if (aBarRef.current) aBarRef.current.style.width = '38%';
        }, 300),
      );
      timers.push(
        window.setTimeout(() => {
          animateCount(bRef.current, 61, 1000, '%');
          if (bBarRef.current) bBarRef.current.style.width = '61%';
        }, 500),
      );
      timers.push(window.setTimeout(() => setShowRec(true), 2000));
    };
  ...
  return (
    <div
      className={`flex aspect-square size-full items-center justify-center bg-[var(--v2-yellow)] p-5 sm:p-6 ${className}`}
      aria-hidden
    >
      <div className="w-full max-w-[340px] rounded-[20px] border-2 border-[var(--v2-ink)] bg-white p-5 shadow-[8px_8px_0_var(--v2-ink)] sm:p-6">
        ...
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-1.5 text-[13px] font-extrabold text-[var(--v2-ink)]">
            <span className="concept-live-dot inline-block size-2.5 shrink-0 rounded-full bg-[var(--v2-orange)]" />
            CONCEPT TEST · LIVE
          </div>
          <div className="font-mono text-sm font-medium text-[#2F5DE0]">
            n=<span ref={nRef}>0</span>
          </div>
        </div>

        <div className="mb-4 flex gap-3">
          <div className="flex-1 rounded-[14px] border-2 border-[var(--v2-ink)] bg-[#f5f5f4] p-4">
            <div className="mb-2.5 text-[12.5px] font-extrabold text-[var(--v2-ink)]">
              Concept A
            </div>
            <div
              ref={aRef}
              className="font-mono text-[29px] font-medium leading-none text-[var(--v2-ink)]"
            >
              0%
            </div>
            <div className="mb-2 mt-1 text-[11.5px] text-[#666]">Purchase intent</div>
            <div className="h-[5px] overflow-hidden rounded-[3px] bg-[#ddd]">
              <div
                ref={aBarRef}
                className="h-full w-0 bg-[var(--v2-ink)] transition-[width] duration-[1400ms] ease-[cubic-bezier(.16,1,.3,1)]"
              />
            </div>
          </div>

          <div className="flex-1 rounded-[14px] border-2 border-[var(--v2-ink)] bg-[var(--v2-orange)] p-4 shadow-[5px_5px_0_var(--v2-ink)]">
            <div className="mb-2.5 text-[12.5px] font-extrabold text-white">Concept B</div>
            <div
              ref={bRef}
              className="font-mono text-[29px] font-medium leading-none text-white"
            >
              0%
            </div>
            <div className="mb-2 mt-1 text-[11.5px] text-[#ffe3d6]">Purchase intent</div>
            <div className="h-[5px] overflow-hidden rounded-[3px] bg-white/35">
              <div
                ref={bBarRef}
                className="h-full w-0 bg-white transition-[width] duration-[1400ms] ease-[cubic-bezier(.16,1,.3,1)]"
              />
            </div>
          </div>
        </div>
        ...
        <div
          className={`flex items-center justify-between rounded-[14px] border-2 border-[var(--v2-ink)] bg-[var(--v2-ink)] px-4 py-4 shadow-[6px_6px_0_var(--v2-orange)] transition-all duration-500 ${
            showRec ? 'translate-y-0 opacity-100' : 'translate-y-2 opacity-0'
          }`}
        >
          <span className="text-sm font-extrabold text-white">Recommendation</span>
          <span className="text-sm font-extrabold text-[var(--v2-orange)]">Go · Concept B</span>
        </div>
      </div>
    </div>
  );
